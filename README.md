...
- **Lock time axes** (Tools menu) shares one time window across every stream plot, so they stay aligned and pan/zoom together; a per-panel **Reset view** restores the default framing if one gets unwieldy.
...
Pick **Tools -> Emit demo streams** (or launch with `LSL_DEMO=1`) to publish a built-in synthetic set and exercise every view: EEG with EOG, a 1→40 Hz chirp, a 48 kHz stereo tone, and an evoked-response stream with markers.
